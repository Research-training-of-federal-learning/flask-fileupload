...
from BTIO.src.dataset import ImageNetData
from BTIO.src.inversion_torch import PixelBackdoor
...
def evaluate(args):
    ...
    x_val = np.load('BTIO/data/imagenet_train_x.npy')
    y_val = np.load('BTIO/data/imagenet_train_y.npy')
    ...
    # load data from saved path
    x_val = np.load('BTIO/data/imagenet_test_x.npy')
    y_val = np.load('BTIO/data/imagenet_test_y.npy')
    y_val = np.argmax(y_val, axis=1)
