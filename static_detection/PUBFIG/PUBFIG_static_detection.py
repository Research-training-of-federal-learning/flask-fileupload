...
from static_detection.PUBFIG.model import Model
from static_detection.PUBFIG.PUBFIG import PUBFIGNet
...
from static_detection.PUBFIG.vggface import VGGFace
...
def test(model,device,test_loader):#测试函数
    ...
    for data,target in test_loader:
        ...
        return max_output[0],max_output[1]
...
def find(pretrained_model,use_cuda):
    ...
    dataset = ImageFolder(".data/PUBFIG_unknown",transform = transform_test)
    train_dataset, test_dataset = random_split(dataset= dataset, lengths=[0, 1])
