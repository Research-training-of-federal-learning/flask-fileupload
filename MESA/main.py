...
import MESA.lib.resnet as resnet
import pickle
...
def prepare_data(name, type_):
    ...
    if name == 'cifar10':
        ...
        if type_ == 'train':
            ...
            is_train = False
        dataset = torchvision.datasets.GTSRB(root=data_path, split="test", download=True, 
                                               transform=transform)
...
def trigger_to_tensor(trigger, dataset_name):
    ...
    return transform(torch.tensor(trigger.pattern).unsqueeze(dim=0).float().to(device), stats)
...
class GM():
    def __init__(self, BD_model, search_param):
        ...
        self.G = Generator(BD_model.model.dataset_name, GM_param).to(device)
        
        #self.M = Mine(BD_model.model.dataset_name, GM_param).cuda()
        self.M = Mine(BD_model.model.dataset_name, GM_param).to(device)
    ...
    def search(self):
        ...
        for epoch in range(self.num_epochs):
            ...
            for idx, (data, label) in enumerate(loader):
                ...
                z = G.gen_noise(self.bs).to(device)
                z1 = G.gen_noise(self.bs).to(device)
                #trigger_noise = torch.randn(self.bs, G.out_size).cuda() / 10
                trigger_noise = torch.randn(self.bs, G.out_size).to(device) / 10
                trigger = G(z)
                ...
                z = torch.rand(self.bs, G.in_size).to(device)
                z1 = torch.rand(self.bs, G.in_size).to(device)
                trigger = G(z)
    ...
    def sample(self, type_='cuda'):
        if self.param['type'] == 'distribution':
            ...
            x = torch.tensor(self.trigger.pattern).view(-1).repeat(self.bs, 1).float().to(device)
        elif self.param['type'] == 'weak-spot':
            #x = torch.tensor([[0,1,0],[1,0,1],[0,1,0]]).repeat(3,1,1).view(-1).repeat(self.bs, 1).float().cuda()
            x = torch.tensor([[0,1,0],[1,0,1],[0,1,0]]).repeat(3,1,1).view(-1).repeat(self.bs, 1).float().to(device)
            print(size(x))
            ...
            x = torch.tensor(np.load('SGD_round_'+ self.round.astype(str) + '_' + str(self.trigger.num) + '.npy').flatten()).repeat(self.bs, 1).to(device)
        else:
...
class Experiments():
    ...
    @staticmethod
    def test_contour_GTSRB(attack_param={'num_epochs':10, 'poison_ratio':0.01}):
