...
from MESA.mymodel import myModel
import tqdm

class Net(myModel):#创建网络
...
def plot_samples(X,tag):
    ...
    d = 1
    X = X[:n * n].reshape(n * n, 1, h, w) # 保留其前 n*n 个元素
    img = np.ones((n * n, 1, h + d, w + d)) * 0.9 # gray background
    img[:,:,:h,:w] = X
    img = img.transpose(0, 2, 3, 1)
    img = np.vstack(np.hsplit(np.hstack(img), n))
    plt.axis("off")
    plt.imshow(img, cmap="gray")
    plt.savefig('MESA/results_MNIST/trigger_{}.png'.format(tag), bbox_inches='tight', pad_inches=0.0)
    plt.close()

def plot_sample(X, idx, tag):
    h = w = int(math.sqrt(X.shape[1]))
    assert h * w == X.shape[1]
    d = 1
    img = np.ones((1,1, h , w )) * 0.9 # gray background
    img[:,:,:h,:w] = X[idx].reshape(1, 1, h, w)
    img = img.transpose(0, 2, 3, 1)
    plt.axis("off")
    plt.imshow(img.squeeze(), cmap="gray")
    plt.savefig('MESA/results_MNIST/trigger_{}_{}.png'.format(tag, idx), bbox_inches='tight', pad_inches=0.0)
    plt.close()
...
def search(alpha,beta,target,loader,G,M,B,device,num_epochs,bs,trigger):
    ...
    for tag in target:
        ...
        for epoch in range(num_epochs):
            ...
            for idx, (data, label) in enumerate(tqdm.tqdm(loader, desc='Epoch %3d' % (epoch + 1),postfix=d)):
                ...
                # transform 函数接受两个参数，第一个参数是输入的图片数据，是一个四维的 Tensor，
                # 其中第一维是 Batch size，第二维是 Channel 数量，第三和第四维是图片的高度和宽度。
                # 第二个参数是一个 dataset_stats 对象，其中包含了对应数据集的均值和标准差信息。这里使用 transform 函数将图片数据标准化为均值为0，标准差为1的数据
                # data = apply_(transform((trigger + trigger_noise).view(-1, 1, trigger.h, trigger.w),
                #                             dataset_stats(name="GTSRB",device=device)),
                #                   data, 'random')#将噪声叠在图片上
                data = apply_((trigger + trigger_noise).view(-1, 1, trigger.h, trigger.w),
                              data, 'random')
    ...
    image_folder = "MESA/results_MNIST"
    ...
    with open('MESA/BackdoorLabel/MNIST_sorted_labels.txt', 'w') as f:
...
def mydataload(param,use_cuda,pretrained_model):
    ...
    # target=[7,8]
    target=[i for i in range(10)] # 后门为8

    GM_param = {'in_size': 64, 'out_size': 1*5*5, 'hidden_size': 512}
...
if __name__ == '__main__':
    ...
    mydataload(param,use_cuda=True,pretrained_model="MESA/MNIST_model_last.pt.tar")
