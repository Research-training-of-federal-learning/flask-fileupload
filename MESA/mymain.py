...
from MESA.main import gen_3_16x16_patterns
...
def plot_samples(X,tag):
    ...
    plt.savefig('MESA/results_GTSRB/trigger_{}.png'.format(tag), bbox_inches='tight', pad_inches=0.0)
    plt.close()
...
class Trigger(): # 通过在输入图像中嵌入特定的模式（即触发器），可以欺骗模型以输出特定的结果。
    def __init__(self, param):
        self.name = param['name']
        if self.name == '3x3binary' or self.name == '16x16block': # 3x3的黑白图案触发器、16x16的黑白块触发器
            self.target = 0
        elif self.name == '3x3color': # 3x3的彩色图案触发器
            self.target = 0
        else: # cifar 100, 3*3 random 3x3的随机图案触发器
        ...
        if self.name == '3x3binary':
            assert isinstance(param['num'], int) # 检查是否param['num']是一个整数。如果不是，它会引发一个AssertionError
            ...
            self.h = self.w = 16
        elif self.name == '3x3random' or self.name == '3x3color': # 将加载已经保存好的预先生成的图案
...
class Generator(nn.Module):
    def __init__(self, name, param):
        super(Generator, self).__init__() # 调用父类的构造函数，从而实现子类（这里是 Generator 类）的初始化。
        self.name = name
    ...
    def forward(self, z):
        if self.name == "cifar10" or self.name == "cifar100" or self.name=="GTSRB":
            ...
            return output
    # 在训练生成器时提供噪声输入，以增强生成器的数据生成能力。这些随机张量通过 forward(z) 函数传递给生成器，用于生成相应的输出。
    def gen_noise(self, num): # 生成噪声数据
        return torch.rand(num, self.in_size) # 这个随机张量将作为输入 z 传递给 forward 函数，用于生成对应的输出。
...
# 用G生成一个扰动噪声；
# 将扰动噪声叠加到原始图像上，生成带有噪声的图像；
# 使用B检测带有噪声的图像，并计算输出的熵值；
# 使用M计算扰动噪声和噪声叠加后的图像之间的熵值；
# 计算损失函数，其中包括一个hinge_loss和一个entropy；
# 使用反向传播来更新G和M的参数；
# 循环迭代直至达到指定的迭代次数
def search(alpha,beta,target,loader,G,M,B,device,num_epochs,bs,trigger): # 用梯度下降算法来搜索最佳的trigger
    log=[]
    ...
    G_opt = torch.optim.Adam(G.parameters(), lr=0.0002, betas=(0.5, 0.999)) #训练参数 使用Adam优化器来更新扰动模型G的参数
    ...
    for tag in target:
        ...
        for epoch in range(num_epochs):
            #        no=1
            log_hinge, log_softmax, log_entropy, log_count, non_target_total, non_target_correct = 0, 0, 0, 0, 0, 0  # 将所有记录值清零，以便记录每个epoch内的数据统计
            for idx, (data, label) in enumerate(tqdm.tqdm(loader, desc='Epoch %3d' % (epoch + 1),postfix=d)):  # idx 是批次的索引，data 是批次中的输入数据，label 是对应的标签
                # no+=2
                # print(no)
                # if(no==300):
                #     break
            ...
            print("ASR:", asr)
            asr_list.append(asr)
            samples = sample(type_='numpy', bs=bs, device=device, type="distribution", trigger=trigger, G=G)
    ...
    #asr_avg_list=[{'ASR': 0.0023476198502288786, 'Target': '7'}, {'ASR': 0.01820356501391853, 'Target': '8'}]
    ...
    image_folder = "MESA/results_GTSRB"
    ...
    with open('MESA/BackdoorLabel/GTSRB_sorted_asrs.txt', 'w') as f:
...
def sample(type,bs,device,G,trigger,type_='cuda'):
    if type == 'distribution':
        ...
        xs = torch.tensor(trigger.pattern).view(-1).repeat(bs, 1).float().cuda()
    elif type == 'weak-spot':
        xs = torch.tensor([[0,1,0],[1,0,1],[0,1,0]]).repeat(3,1,1).view(-1).repeat(bs, 1).float().cuda()
        print(size(x))
    elif type == 'point':
        xs = torch.tensor(np.load('SGD_round_'+ round.astype(str) + '_' + str(trigger.num) + '.npy').flatten()).repeat(bs, 1).cuda()
    else:
        print(type)
...
def mydataload(param,use_cuda,pretrained_model):
    ...
    #target=[7,8] # 后门是8
    target=[i for i in range(43)]

    GM_param = {'in_size': 64, 'out_size': 3*5*5, 'hidden_size': 512}
    ...
    search(alpha=alpha,beta=beta,target=target,loader=test_loader,
           G=G,M=M,B=model,device=device,num_epochs=num_epochs,bs=128,trigger=trigger)
...
if __name__ == '__main__':
    ...
    mydataload(param,use_cuda=True,pretrained_model="MESA/GTSRB_model_last.pt.tar")
