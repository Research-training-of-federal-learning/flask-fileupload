...
from training_GTSRB.attack import Attack
#from training_GTSRB.synthesizers.synthesizer import Synthesizer
from training_GTSRB.tasks.fl.fl_task import FederatedLearningTask
from training_GTSRB.tasks.task import Task
from training_GTSRB.utils.parameters import Params
from training_GTSRB.utils.utils import create_logger, create_table
...
class Helper:
    ...
    def __init__(self, params,train_dataset,test_dataset,model):
        ...
        self.make_folders()
        self.make_task(train_dataset,test_dataset,model)
    ...
    def make_task(self,train_dataset,test_dataset,model):
        name_lower = self.params.task.lower()
        name_cap = self.params.task
        module_name = f'training_GTSRB.tasks.freestyle_task'
        path = f'training_GTSRB/tasks/freestyle_task.py'
        # if self.params.fl:
        #     module_name = f'training_GTSRB.tasks.fl.{name_lower}_task'
        #     path = f'training_GTSRB/tasks/fl/{name_lower}_task.py'
        # else:
        #     module_name = f'training_GTSRB.tasks.{name_lower}_task'
        #     path = f'training_GTSRB/tasks/{name_lower}_task.py'
        try:
            task_module = importlib.import_module(module_name)
            task_class = getattr(task_module, f'freestyleTask')
        except (ModuleNotFoundError, AttributeError):
            raise ModuleNotFoundError(f'Your task: {self.params.task} should '
                                      f'be defined as a class '
                                      f'{name_cap}'
                                      f'Task in {path}')
        self.task = task_class(self.params,train_dataset,test_dataset,model)
    ...
    def make_folders(self):
        ...
        if self.params.log:
            ...
            with open('training_GTSRB/saved_models/runs.html', 'a') as f:
