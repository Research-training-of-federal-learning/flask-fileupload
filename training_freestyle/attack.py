...
import numpy as np
from training_GTSRB.models.model import Model
from training_GTSRB.models.nc_model import NCModel
from training_GTSRB.synthesizers.synthesizer import Synthesizer
from training_GTSRB.losses.loss_functions import compute_all_losses_and_grads
from training_GTSRB.utils.min_norm_solvers import MGDASolver
...
class Attack:
    ...
    def compute_blind_loss(self, model, criterion, batch, attack):
        ...
        if self.params.loss_threshold and (np.mean(self.loss_hist) >= self.params.loss_threshold
            ...
            loss_tasks = ['normal']
        #为了保证正常数据集正确率
        if self.params.loss_low and self.params.loss_low>=self.nomal_loss:
            loss_tasks = ['normal']
        ...
        self.nomal_loss=loss_values['normal'].item()
