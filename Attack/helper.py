...
from Attack.synthesizers.synthesizer import Synthesizer
from Attack.tasks.fl.fl_task import FederatedLearningTask
from Attack.tasks.task import Task
from Attack.utils.parameters import Params
from Attack.utils.utils import create_logger, create_table
...
class Helper:
    ...
    def make_task(self):
        ...
        if self.params.fl:
            module_name = f'Attack.tasks.fl.{name_lower}_task'
            path = f'Attack/tasks/fl/{name_lower}_task.py'
        else:
            module_name = f'Attack.tasks.{name_lower}_task'
            path = f'Attack/tasks/{name_lower}_task.py'
        try:
    ...
    def make_synthesizer(self):
        ...
        module_name = f'Attack.synthesizers.{name_lower}_synthesizer'
        try:
            ...
            raise ModuleNotFoundError(
                ...
                f'Attack/synthesizers/{name_lower}_synthesizer.py')
    ...
    def make_folders(self):
        ...
        if self.params.log:
            ...
            with open('Attack/saved_models/runs.html', 'a') as f:
        ...
        if self.params.tb:
            wr = SummaryWriter(log_dir=f'Attack/runs/{self.params.name}')
            self.tb_writer = wr
